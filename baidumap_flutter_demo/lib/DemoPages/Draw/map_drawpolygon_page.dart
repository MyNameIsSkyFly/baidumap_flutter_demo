...
class _DrawPolygonPageState extends BMFBaseMapState<DrawPolygonPage> {
          ...
          Container(
            ...
            child: Row(
              children: [
                Container(
                  width: 80,
                  child: Text("StrokeColor", style: TextStyle(color: Colors.white)),
                ),
                Flexible(
                  child: Slider(
          ...
          Container(
            ...
            child: Row(
              children: [
                Container(
                  width: 80,
                  child: Text("FillColor", style: TextStyle(color: Colors.white)),
                ),
                Flexible(
                  child: Slider(
  ...
  void reset() {
    ...
    if (Get.isRegistered<DrawBoardController>()) {
      Get.find<DrawBoardController>().reset();
    }
    myMapController.removeOverlay(_polygon0.id);
  }

  drawMap(List<Offset> list) async {
    setState(() {
      showDrawBoard = false;
    });
    if (Get.isRegistered<DrawBoardController>()) {
    ...
    for (var element in list) {
      BMFCoordinate? coor = await myMapController.convertScreenPointToCoordinate(BMFPoint(element.dx, element.dy));
      coordinates.add(coor!);
    }
    BMFPolygon polygon = BMFPolygon(coordinates: coordinates, strokeColor: Colors.blue, width: 4, fillColor: Colors.blue.withOpacity(0.4));
    _polygon0 = polygon;
    await myMapController.addPolygon(polygon);
  }
  ...
  void _onPressedDash() {
    if (Platform.isAndroid) {
      ...
      _polygon0 = BMFPolygon(
          coordinates: coordinates, strokeColor: Colors.blueGrey, width: 5, fillColor: Colors.red, lineDashType: BMFLineDashType.LineDashTypeSquare);
  ...
  void _onPressedCircleHollow() {
    ...
    BMFHollowShape circleHollowShape1 = BMFHollowShape.circle(center: center1, radius: 1500);
    ...
    BMFHollowShape circleHollowShape2 = BMFHollowShape.circle(center: center2, radius: 1500);
  ...
  void _onPressedPolygonHollow() {
    ...
    BMFHollowShape polygonHollowShape1 = BMFHollowShape.polygon(coordinates: coordinates1);
    ...
    BMFHollowShape polygonHollowShape2 = BMFHollowShape.polygon(coordinates: coordinates2);
  ...
  void _onChangedStrokeColorSlider(value) {
    ...
    _polygon0.updateStrokeColor(Colors.blueGrey.withAlpha((255 * value).toInt()));
  }
  ...
  void addPolygon0() {
    ...
    dashCenter = center;
    _polygon0 = BMFPolygon(coordinates: coordinates, strokeColor: Colors.blueGrey, width: 5, fillColor: Colors.red);
    myMapController.addPolygon(_polygon0);
  }
...
class DrawBoard extends StatelessWidget {
  const DrawBoard({Key? key, this.finishCallBack}) : super(key: key);
  final Function(List<Offset> list)? finishCallBack;

  @override
  Widget build(BuildContext context) {
    final _controller = Get.put(DrawBoardController());

    return GestureDetector(
      onPanStart: _controller.onPanStart,
      onPanUpdate: _controller.onPanUpdate,
      onPanEnd: (d) {
        if (_controller.points.length > 2 && finishCallBack != null) {
          finishCallBack!(_controller.points.map((element) => element!.offset).toList());
        }
      },
...
class DrawBoardController extends GetxController {
  ...
  void onPanUpdate(DragUpdateDetails d) {
    if (d.delta == Offset.zero || distance(points.last!.offset, d.localPosition) > 20) return;
    final model = _Model(offset: d.localPosition, paint: paint);
    points.add(model);
  }

  void onPanEnd(DragEndDetails d) {
    // final model = _Model(offset: points.first!.offset, paint: paint);
    // points.add(model);
    // points.add(null);
  }
